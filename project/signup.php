...
<body>
    <header class="text-center">
        ...
        <form autocomplete="off" method="POST" onsubmit="return email.value!=''">
            ...
            <div><input type="password" name="pass" onfocus="this.type = 'text'" onblur="this.type = 'password'" placeholder="Password"></div>
            <div><input type="file" id="file" style="border:0;"></div>
            <?php if(isset($_GET["been"])){echo "<span style='color:red'>Email is already in use</span>";} ?>
            <script>
            ...
            function handleFileSelect(e) {
                ...
                reader.onload = function(e) {
                    ...
                    document.getElementById("iid").value = base64;
                };
                reader.readAsDataURL(f);
                });
            }
        </script>
            <input type="hidden" id="iid" name="file">
            <div><button type="submit" name="send" style="margin-top:20px;">Create New Account</button></div>
            <div style="margin-top:20px;font-size:16px;font-family:Arial, Helvetica,sans-serif;color:#444;">OR</div>
            <div><button type="button" onclick="window.location.href='log.php'" style="margin-top:20px;border:1px solid #e0e0e0;color:#4f84f4;background:#fff;">Log In</button></div>
        </form>
        <?php
            if(isset($_POST["send"])){
                $email = $_POST["email"];
                ...
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                      if($email == $row["email"]){
                        die(
                            print("<script>location.href = 'signup.php?been'</script>")
                        );
                      }
                    }
                  } else {
                ...
                $sql = "INSERT INTO `db` VALUES ('NULL', '$email', '$pass', '$ver', '$fname', '$lname', '$file')";
